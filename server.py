...
def build_excel_from_data(audit_data: dict, entity: str, period: str,
    ...
    for sdef in sheets:
        ...
        if stype == "审定表" or stype == "table":
            ...
            builder.add_核查表(sdef)
        elif stype == "明细":
            builder.add_明细表(sdef)
        else:
            builder.add_审定表(sdef)  # 默认按审定表渲染
...
async def demo_result(subject: str = "C"):
    demos = {
        "C": {"subject":"C","subject_name":"货币资金","entity_name":"重庆蛮先进智能制造有限公司","period":"2024-12-31",
              "summary":"货币资金期末余额6,055,000元。发现：①工行渝北支行存在银行已收企业未记账利息3,500元、银行已付管理费1,200元，调节后余额一致；②其他货币资金300,000元保证金需函证确认；③库存现金盘点差异-900元需查明原因。审计结论：保留意见（部分账户函证程序缺失）。",
              "sheets":["C-1 货币资金审定表","C-2 库存现金盘点表","C-3 银行存款余额调节表","C-4 银行函证汇总表","C-5 大额流水核查表"],
              "audit_findings":[{"severity":"warning","msg":"工行渝北支行存在未达账项（银收企未记3,500元、银付企未记1,200元）"},{"severity":"error","msg":"建行、招行等账户未取得银行对账单，无法执行余额调节程序"},{"severity":"warning","msg":"库存现金盘点差异-900元（盘点54,100 vs 账面55,000）"},{"severity":"warning","msg":"其他货币资金-保证金300,000元未函证，需发出银行询证函"},{"severity":"info","msg":"12月31日大额转账至建行500,000元，关注期后资金流向"}]},
        "D": {"subject":"D","subject_name":"应收票据","entity_name":"重庆蛮先进智能制造有限公司","period":"2024-12-31",
              "summary":"应收票据期末余额2,700,000元（银承1,800,000元、商承900,000元）。发现：①商承PJ-007（135,000元）已逾期未兑付，需转入应收账款并评估坏账；②商承PJ-003（265,000元）信用等级较低需关注；③已贴现银承620,000元终止确认合规。审计结论：保留意见（逾期商承减值测试需进一步执行）。",
              "sheets":["D-1 应收票据审定表","D-2 应收票据明细表","D-3 票据盘点函证表"],
              "audit_findings":[{"severity":"error","msg":"商承PJ-007（135,000元）已逾期，需转入应收账款并减值测试"},{"severity":"warning","msg":"商承PJ-003（265,000元）信用等级较低，关注兑付风险"},{"severity":"warning","msg":"已背书银承PJ-006（850,000元）未到期，需检查是否满足终止确认条件"},{"severity":"info","msg":"已贴现银承620,000元，会计处理符合终止确认条件"}]},
        "E": {"subject":"E","subject_name":"其他应收款","entity_name":"重庆蛮先进智能制造有限公司","period":"2024-12-31",
              "summary":"其他应收款期末余额965,000元。发现：①股东王五借款500,000元（关联方资金占用），需检查审批程序、利息计提及附注披露；②前员工李四借款30,000元已挂账25个月，应全额计提坏账准备；③重庆鑫源装备保证金50,000元（关联方）需披露。审计结论：保留意见（关联方资金占用和长账龄坏账问题）。",
              "sheets":["E-1 其他应收款审定表","E-2 其他应收款明细表","E-3 坏账准备测试表"],
              "audit_findings":[{"severity":"error","msg":"股东王五借款500,000元（关联方资金占用），需检查审批和披露"},{"severity":"error","msg":"前员工李四个人借款30,000元挂账25个月，应全额计提坏账"},{"severity":"warning","msg":"重庆鑫源装备保证金50,000元（关联方），需在附注中披露"},{"severity":"warning","msg":"四川恒力机械资金拆借30万元，需检查借款协议及利息条款"}]},
        "F": {"subject":"F","subject_name":"预付账款","entity_name":"重庆蛮先进智能制造有限公司","period":"2024-12-31",
              "summary":"预付账款期末余额2,660,000元。发现：①华为技术预付850,000元（100%付款），货物未到，需关注商业合理性和合同条款；②重庆渝快物流预付180,000元挂账14个月，关注是否无法履约需计提减值；③中石化重庆分公司预付250,000元（50%付款），需按到货进度冲减。审计结论：保留意见（大额全额预付和长期挂账需进一步核查）。",
              "sheets":["F-1 预付账款审定表","F-2 预付账款明细表","F-3 大额预付函证表"],
              "audit_findings":[{"severity":"error","msg":"华为技术850,000元全额预付未到货，关注商业合理性和合同条款"},{"severity":"error","msg":"重庆渝快物流180,000元挂账14个月，关注是否无法履约"},{"severity":"warning","msg":"重庆钢铁设备480,000元（付款40%），需核对到货进度按比例冲减"},{"severity":"info","msg":"京东企业购已到货结算，处理正常"}]},
        "G": {"subject":"G","subject_name":"预收账款","entity_name":"重庆蛮先进智能制造有限公司","period":"2024-12-31",
              "summary":"预收账款期末余额5,075,000元。发现：①赛力斯集团1,800,000元（100%收款）未交付，关注是否推迟确认收入（舞弊风险）；②长安福特420,000元挂账15个月，需查明原因；③重庆长安汽车1,250,000元（50%收款）部分交付，需按履约进度确认收入。审计结论：保留意见（大额全额预收长期未交付，关注收入确认舞弊风险）。",
              "sheets":["G-1 预收账款审定表","G-2 预收账款明细表","G-3 收入确认测试表"],
        ...
        "H": {"subject":"H","subject_name":"短期借款","entity_name":"重庆蛮先进智能制造有限公司","period":"2024-12-31",
              "summary":"短期借款期末余额19,500,000元。发现：①重庆农商行利息差异3,000元（应计23,000 vs 账面20,000），需补提；②招行重庆分行2,000,000元本期已到期，需确认续贷或还款计划；③抵押借款共5,000,000元（厂房+设备），需检查抵押合同及资产所有权。审计结论：保留意见（利息差异需补提，到期借款需确认续贷）。",
              "sheets":["H-1 短期借款审定表","H-2 短期借款明细表","H-3 利息测算表"],
